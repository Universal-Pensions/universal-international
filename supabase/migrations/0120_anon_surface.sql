-- =============================================================================
-- Universal Pensions Uganda — 0120: bound the anonymous (pre-login) surface
-- =============================================================================
-- Audit 2026-08-23, agent A03 (privilege surface). Closes A03-001 (high),
-- A03-002 (low), A03-003 (low), A03-004 (info).
--
-- WHAT ANON CAN ACTUALLY REACH
-- 13 public functions carry EXECUTE for `anon`, but 10 of them are trigger
-- functions (RETURNS trigger — PostgREST cannot call those) and only three are
-- callable RPCs:
--     get_employer_invite(text)
--     create_subscriber_from_signup(jsonb, text)
--     create_subscriber_from_employer_invite(jsonb, text, text)
-- All three are anon-reachable BY DESIGN: a prospective member has no account
-- yet, so the landing / signup / invite journey has no session to authenticate
-- with. NOTHING IS REVOKED HERE. The fix bounds what those RPCs will DO.
--
-- ── A03-001 (high) — an invite token was not bound to the invited person ─────
-- create_subscriber_from_employer_invite chose its victim from the CALLER'S
-- payload phone and never compared it against employer_invites.prefill->>'phone'
-- (the number the employer actually minted the invite for). Its middle branch
-- then ran an unconditional
--     UPDATE subscribers SET employer_id = <invite's employer> WHERE id = <that row>
-- followed, on every branch, by an unconditional overwrite of that subscriber's
-- `compensation`. So a holder of any single valid pending token — and invite
-- links travel over SMS/WhatsApp — could name any of the 5,006 live subscribers
-- who have no employer, silently move that real person onto an employer's roster
-- and rewrite their stated monthly pay. No session required; the function is
-- SECURITY DEFINER so its UPDATE bypasses RLS.
-- FIX: require the payload phone to equal the invite's own prefill phone
-- (compared on the last 9 digits). A token can now only ever act on the person
-- it was issued for. The mint path (create_employer_invite) has required a valid
-- prefill phone since 0092, so every real invite still completes.
--
-- ── A03-002 (low) — client-trusted phone canonicalisation ───────────────────
-- The RPCs accepted a bare 9-digit phone and stored it verbatim, while login
-- (api/_lib/phone.ts toCanonicalUGPhone -> api/auth/_lib/personas.ts
-- resolveSubscriber) looks the row up by the exact '+256XXXXXXXXX' string and
-- falls back to the s-0001 demo persona when it finds nothing. An account minted
-- with a bare phone was therefore unreachable by its own owner.
-- FIX: canonicalise inside the write, not in the caller.
--
-- ── A03-003 (low) — unbounded free text ────────────────────────────────────-
-- subscribers.name/email/phone/nin/occupation are all uncapped `text` and the
-- anon insert path wrote them verbatim (a 1,000,009-character name was observed
-- persisting through this exact path in round 1). FIX: bounds in the RPCs plus
-- CHECK constraints on the table itself, so the floor holds for every writer.
-- Caps are generous — the longest value in the 5,064 live rows is 35 chars.
--
-- ── A03-004 (info) — sequence grants ───────────────────────────────────────-
-- commission_id_seq / subscriber_id_seq still carried the Supabase-default
-- anon+authenticated rwU. Unreachable (PostgREST never exposes nextval/setval,
-- and every nextval here runs inside a SECURITY DEFINER function as postgres),
-- but the two *_log sequences were already revoked in 0080 — match them.
--
-- SAFETY: no data is rewritten. The four live invite rows all carry a valid
-- prefill phone and all 5,064 subscriber rows are far inside the new caps, so
-- both the CHECK constraints and the invite binding are satisfied by the
-- existing data as-is.
-- =============================================================================
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 1. A03-001 (+ A03-002, A03-003 on this path)
--    create_subscriber_from_employer_invite — bind the token to the invited
--    phone. Body below is the LIVE definition with the guards inserted; nothing
--    else in it was changed.
-- ─────────────────────────────────────────────────────────────────────────────
...
DECLARE
  ...
  v_inv_phone_norm text;  -- 0120 / A03-001: the phone this invite was minted FOR
...
BEGIN
  ...
  -- ── 0120 / A03-001 ────────────────────────────────────────────────────────
  -- An invite token is a bearer credential that travels over SMS/WhatsApp.
  -- Before this guard the whole function keyed on the CALLER-SUPPLIED phone and
  -- never once consulted the phone the invite was actually issued for, so any
  -- holder of any valid pending token could name someone else's number and the
  -- branch below would re-home that real, unaffiliated subscriber into an
  -- employer that never invited them and overwrite their compensation with this
  -- invite's prefill. Bind the two together: a token may only ever act on the
  -- person it was minted for. Compared on the last 9 digits so '0701234567',
  -- '701234567' and '+256701234567' are all the same number (create_employer_invite
  -- stores whatever the employer typed; the completion UI sends the +256 form).
  v_inv_phone_norm := right(regexp_replace(COALESCE(v_inv.prefill ->> 'phone',''),'[^0-9]','','g'),9);
  IF char_length(v_inv_phone_norm) <> 9 THEN
    -- create_employer_invite has required a valid prefill phone since 0092, so
    -- this can only be a hand-written row. Refuse rather than fall back to
    -- trusting the caller — an unbound token is exactly the hole above.
    RAISE EXCEPTION 'this invite is missing the phone number it was sent to and cannot be completed' USING ERRCODE='P0001';
  END IF;
  IF v_phone_norm <> v_inv_phone_norm THEN
    RAISE EXCEPTION 'this invite was sent to a different phone number' USING ERRCODE='P0001';
  END IF;

  -- ── 0120 / A03-003 ────────────────────────────────────────────────────────
  -- Bound the free-text fields on the anon-reachable path. Rejects an over-long
  -- value only; it never makes a field required (an absent key is '').
  IF char_length(COALESCE(payload ->> 'fullName', '')) > 120 THEN RAISE EXCEPTION 'fullName is too long (max 120 characters)' USING ERRCODE='22001'; END IF;
  IF char_length(COALESCE(payload ->> 'email', '')) > 254 THEN RAISE EXCEPTION 'email is too long (max 254 characters)' USING ERRCODE='22001'; END IF;
  IF char_length(COALESCE(payload ->> 'phone', '')) > 20 THEN RAISE EXCEPTION 'phone is too long (max 20 characters)' USING ERRCODE='22001'; END IF;
  IF char_length(COALESCE(payload ->> 'nin', '')) > 40 THEN RAISE EXCEPTION 'nin is too long (max 40 characters)' USING ERRCODE='22001'; END IF;
  IF char_length(COALESCE(payload ->> 'occupation', '')) > 120 THEN RAISE EXCEPTION 'occupation is too long (max 120 characters)' USING ERRCODE='22001'; END IF;
  ...
  ELSE
    ...
    -- 0120 / A03-002: store the canonical '+256XXXXXXXXX' form. Login
    -- (api/_lib/phone.ts toCanonicalUGPhone) resolves the row by that exact
    -- string, so a bare 9-digit value stored verbatim was an account its owner
    -- could never sign into (it fell through to the s-0001 demo default).
    VALUES (v_new_id, payload ->> 'fullName', NULLIF(payload ->> 'email',''), '+256' || v_phone_norm, payload ->> 'gender',
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 2. A03-002 + A03-003
--    _insert_subscriber_chain — the one shared writer into `subscribers`
--    (create_subscriber_from_signup, create_subscriber_from_agent_onboard, and
--    the invite's collect_schedule branch all funnel through it). Body below is
--    the LIVE definition with the guards inserted; nothing else was changed.
-- ─────────────────────────────────────────────────────────────────────────────
...
DECLARE
  ...
  v_prem_ref       TEXT;
BEGIN
  -- ── 0120 / A03-002 + A03-003 ──────────────────────────────────────────────
  -- This is the single INSERT that reaches `subscribers` for the signup, agent
  -- onboard and employer-invite (collect_schedule) paths. Every caller already
  -- validates the phone shape and none of them cap length; re-check here rather
  -- than trust three callers, and do it before nextval() so a rejected payload
  -- does not burn a subscriber id.
  IF char_length(right(regexp_replace(COALESCE(p_payload ->> 'phone', ''), '[^0-9]', '', 'g'), 9)) <> 9 THEN
    RAISE EXCEPTION 'phone must be a valid Uganda number (9 digits, optional +256 prefix)';
  END IF;
  IF char_length(COALESCE(p_payload ->> 'fullName', '')) > 120 THEN RAISE EXCEPTION 'fullName is too long (max 120 characters)' USING ERRCODE='22001'; END IF;
  IF char_length(COALESCE(p_payload ->> 'email', '')) > 254 THEN RAISE EXCEPTION 'email is too long (max 254 characters)' USING ERRCODE='22001'; END IF;
  IF char_length(COALESCE(p_payload ->> 'phone', '')) > 20 THEN RAISE EXCEPTION 'phone is too long (max 20 characters)' USING ERRCODE='22001'; END IF;
  IF char_length(COALESCE(p_payload ->> 'nin', '')) > 40 THEN RAISE EXCEPTION 'nin is too long (max 40 characters)' USING ERRCODE='22001'; END IF;
  IF char_length(COALESCE(p_payload ->> 'occupation', '')) > 120 THEN RAISE EXCEPTION 'occupation is too long (max 120 characters)' USING ERRCODE='22001'; END IF;
  ...
  INSERT INTO public.subscribers (
    ...
    -- 0120 / A03-002: canonical '+256XXXXXXXXX'. The guard at the top of this
    -- function has already proved there are 9 digits to take.
    '+256' || right(regexp_replace(COALESCE(p_payload ->> 'phone', ''), '[^0-9]', '', 'g'), 9),
    p_payload ->> 'gender',
    v_age,
    v_dob,
    p_payload ->> 'nin',
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 3. A03-003 — hard floor at the table, so the bound holds for every writer
--    (including create_subscriber_from_employer_onboard and any admin path),
--    not only for the two RPCs patched above.
--    Live maxima at time of writing: name 30, email 35, phone 14, nin 14,
--    occupation 22 — every existing row validates immediately.
-- ─────────────────────────────────────────────────────────────────────────────
ALTER TABLE public.subscribers
  ADD CONSTRAINT subscribers_name_len_chk       CHECK (char_length(name)       <= 120),
  ADD CONSTRAINT subscribers_email_len_chk      CHECK (char_length(email)      <= 254),
  ADD CONSTRAINT subscribers_phone_len_chk      CHECK (char_length(phone)      <=  20),
  ADD CONSTRAINT subscribers_nin_len_chk        CHECK (char_length(nin)        <=  40),
  ADD CONSTRAINT subscribers_occupation_len_chk CHECK (char_length(occupation) <= 120);

-- ─────────────────────────────────────────────────────────────────────────────
-- 4. A03-004 — least privilege on the two standalone sequences.
--    Neither backs a column default; both are only ever advanced from inside a
--    SECURITY DEFINER function running as postgres, so this changes no
--    behaviour. It brings them in line with entity_detach_log_id_seq /
--    entity_status_log_id_seq, revoked in 0080.
-- ─────────────────────────────────────────────────────────────────────────────
REVOKE ALL ON SEQUENCE public.commission_id_seq FROM anon, authenticated;
REVOKE ALL ON SEQUENCE public.subscriber_id_seq FROM anon, authenticated;
