-- 0109_settlement_tenancy.down.sql
-- Restores `public.apply_settlement` to the body that was LIVE immediately
-- before 0109, captured verbatim with
--     select pg_get_functiondef('public.apply_settlement'::regproc);
-- (project ilkhfnoyxlxwqadebnkp, 2026-08-25). Not retyped from 0032/0051 — the
-- live body is the only source of truth; retyping from an older migration is
-- how 0095 clobbered 0090's work and shipped a regression to production.
--
-- WARNING — reverting re-opens finding A05-001 (CRITICAL, cross-tenant money
-- movement): any distributor can mark ANY other distributor's agents'
-- commissions paid, stamp them with a payment reference they never issued,
-- notify the victim's agents and branches, and write a settlement_batches row
-- into the victim's branch. Proven live: d-002 settled a-001 (owned by d-001).
--
-- CREATE OR REPLACE keeps the owner (postgres) and the ACL
-- (postgres/authenticated/service_role EXECUTE) — no GRANT is needed here.
