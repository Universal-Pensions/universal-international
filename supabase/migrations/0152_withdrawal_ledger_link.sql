-- 0152_withdrawal_ledger_link.sql
-- ============================================================================
-- THREE MONEY OPERATIONS WERE JOINING ON A KEY THAT IS NOT A KEY.
-- Found by auditing 0147/0148, not by a failure.
--
-- `public.withdrawals` has no foreign key to `public.transactions`. The two rows
-- written by request_withdrawal were related only by a shared `reference` — the
-- string 'WD-' followed by six random digits. Nothing depended on that link
-- before this project, so nobody had ever tested it. 0147 and 0148 then built
-- THREE money operations on top of it:
--
--   * the engine's D8 rejection  (mark the history row 'rejected')
--   * reverse_transaction        (mark it 'reversed')
--   * settle_withdrawal          (find the ledger row to check it was priced)
--
-- MEASURED ON LIVE, and it is not close:
--   4,937 withdrawals total
--   3,731 (76%) have a NULL reference — and `reference = NULL` matches nothing,
--         ever, so those rows are simply invisible to all three operations
--     221 members have DUPLICATE non-null references, up to FOUR withdrawals
--         sharing one value
--       1 reference value appears 1,191 times across the table
--   4,936 of 4,937 withdrawals have NO transaction resolvable by
--         (subscriber_id, reference) at all — exactly ONE links
--
-- THE TWO FAILURES THIS CAUSES
--   1. UNDER-MATCHING. settle_withdrawal's lookup found no ledger row for
--      essentially every historical withdrawal, so its "has this been priced
--      yet?" guard was skipped in silence — a no-match is indistinguishable
--      from a passed check.
--   2. OVER-MATCHING, which is the dangerous one. The D8 and reversal UPDATEs
--      carry no LIMIT, so for the 221 members whose withdrawals share a
--      reference, rejecting or reversing ONE redemption would have flipped up
--      to FOUR unrelated history rows with it. A member would see three
--      withdrawals they never touched marked 'reversed'.
--
-- THE FIX is the column that should always have existed: withdrawals.transaction_id,
-- a real FK, written by request_withdrawal and used by all three operations.
-- No reference-matching fallback is kept, deliberately — a fallback here would
-- reintroduce exactly the over-matching this migration exists to remove, and it
-- is not needed: the only rows these operations can legitimately act on are
-- written after this migration and always carry the link. Historical rows are
-- already unreachable by D8 and by reverse_transaction (both require a
-- units_delta that no pre-0144 row has), and settle_withdrawal now states
-- explicitly when it is settling an unlinked legacy row.
--
-- BACKFILL: none is attempted. Exactly one of 4,937 rows is resolvable, and
-- guessing the other 4,936 from a colliding random string is how you attach a
-- member's withdrawal to someone else's ledger row.
--
-- ROLLBACK: 0152_withdrawal_ledger_link.down.sql restores the reference-matching
-- bodies and drops the column. It reinstates the over-matching bug.
-- ============================================================================
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'withdrawals_transaction_fk') THEN
    ALTER TABLE public.withdrawals
      ADD CONSTRAINT withdrawals_transaction_fk
      FOREIGN KEY (transaction_id) REFERENCES public.transactions(id) ON DELETE SET NULL;
  END IF;
END $$;

-- One history row per ledger row. This is the invariant `reference` never had.
CREATE UNIQUE INDEX IF NOT EXISTS ux_withdrawals_transaction
  ON public.withdrawals (transaction_id) WHERE transaction_id IS NOT NULL;

COMMENT ON COLUMN public.withdrawals.transaction_id IS
  'FK to the transactions row this history row was written alongside. The ONLY reliable link between the two. `reference` is not one: it is NULL on 76% of pre-0152 rows and 221 members share a value across up to four withdrawals. NULL means the row predates 0152.';
...
BEGIN
  ...
  -- 0152: transaction_id is the REAL link between the ledger row and this
  -- history row. `reference` was never one: it is 'WD-' + 6 random digits, it is
  -- NULL on 3,731 of 4,937 existing rows, 221 members have DUPLICATE non-null
  -- references (up to 4 sharing one), and one value repeats 1,191 times. Every
  -- money operation added in 0147/0148 matched on it.
  INSERT INTO public.withdrawals (
    id, subscriber_id, amount, bucket, reason, method, status, date, reference, transaction_id
  ) VALUES (
    v_wd_id, v_subscriber_id, p_amount, v_bucket, p_reason, p_method, 'processing',
    (now())::date, v_ref, v_tx_id
  );
