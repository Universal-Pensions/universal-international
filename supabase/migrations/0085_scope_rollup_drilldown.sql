-- 0085_scope_rollup_drilldown.sql
-- Completes 0082: the region / district / branch / agent levels of
-- get_entity_metrics_rollup are now distributor-scoped too.
--
-- 0082 scoped only the COUNTRY level, because the sibling gap — `agents` and
-- `branches` still being readable platform-wide — meant a scoped drill-down
-- would have been half-filtered: a distributor could still SEE another
-- distributor's branch in a list, then drill in and get zeros. 0084 closed that
-- with RESTRICTIVE policies, so the drill-down can now be scoped consistently.
--
-- Without this, a distributor could pass ANY region/district/branch/agent id to
-- the RPC and get that entity's real metrics back, because the function is
-- SECURITY DEFINER and bypasses the 0081/0084 policies entirely. With two real
-- tenants (d-001 = 289 branches, d-002 = the 27 Busoga branches) that is a live
-- cross-tenant read, not a hypothetical one.
--
-- Mechanism is identical to 0082: a single `v_all` boolean, TRUE for every role
-- except 'distributor', added to each level's scope CTEs. admin / branch / agent
-- results stay byte-identical (branch and agent are additionally pinned by the
-- pre-existing out_of_scope guards). Fails closed on a missing claim.
--
-- Generated by an asserted transform of 0082's body (10 replacements, each with
-- an exact expected match count) and diffed to prove nothing else moved.
    ...
    scope_subscriber AS (
      SELECT d.region_id AS entity_id, s.id AS subscriber_id, s.agent_id,
       ...
       WHERE d.region_id = ANY(p_entity_ids)
         AND (v_all OR b.distributor_id = v_dist)
    ),
    scope_agent AS (
    ...
    scope_subscriber AS (
      SELECT a.branch_id AS entity_id, s.id AS subscriber_id, s.agent_id,
       ...
       WHERE a.branch_id = ANY(p_entity_ids)
         AND (v_all OR a.branch_id IN (SELECT public.distributor_branch_ids()))
    ),
    scope_agent AS (
    ...
    scope_subscriber AS (
      SELECT s.agent_id AS entity_id, s.id AS subscriber_id,
       ...
       WHERE s.agent_id = ANY(p_entity_ids)
         AND (v_all OR s.agent_id IN (SELECT public.distributor_agent_ids()))
    ),
    counts AS (
