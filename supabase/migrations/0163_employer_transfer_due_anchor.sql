-- 0163_employer_transfer_due_anchor.sql
-- ============================================================================
-- "Delayed employer transfers" listed seven employers and could say almost
-- nothing about six of them.
--
-- THE DEFECT
-- ----------
-- Raised, Due by and Days late were ALL derived from the previous run:
--
--     'dueBy',    (lr.last_run::date + _employer_grace_days(...)),
--     'daysLate', CASE WHEN lr.last_run IS NULL THEN NULL ELSE ... END
--
-- while the WHERE clause admitted `lr.last_run IS NULL`. So an employer that
-- has NEVER posted a run was counted in the badge and listed in the drill-down,
-- and then had nothing to compute a date from: every one of those three columns
-- came back NULL and rendered as an em dash. Measured on live, six of seven
-- employers were in exactly that state.
--
-- Two things followed from it:
--   • the tile says "Ranked most overdue first" while ORDER BY lr.last_run
--     NULLS FIRST could not rank never-run employers against each other at all;
--   • the escalation draft degrades to "your run is now due" (attentionMeta's
--     days() returns 'now due' for a null daysLate) for employers who have
--     never run once — the most overdue case gets the softest wording.
--
-- THE FIX
-- -------
-- "Has never posted a run" is not the same fact as "is overdue". The due date
-- now falls back to the employer's FIRST MEMBER ENROLMENT: the first payroll
-- was owed one cadence after there were staff to run it for. Both the count and
-- the row list use the identical anchor expression, so the badge and the list
-- cannot disagree.
--
-- Also fixed by the same anchor: an employer with NO members and no runs is no
-- longer flagged. It has nobody to pay, so it owes nothing — previously it was
-- permanently overdue for a payroll it never owed. (No employer on live is in
-- that state today; this stops the next one being reported wrongly.)
--
-- `date` deliberately stays NULL for a never-run employer — there is no raised
-- date and inventing one would be a lie. A new `neverRun` flag lets the client
-- render "No run" instead of a bare em dash, so the cell reads as a known state
-- rather than as missing data.
--
-- NOT FIXED HERE: the demo seed gives payroll history to emp-001 only
-- (src/data/employerSeed.js:317 builds runs for EMPLOYER.id and nothing else),
-- which is why six employers hit this path at all. Each run carries 42
-- transaction legs, so seeding the rest is money-bearing work and is
-- deliberately not smuggled into a display fix.
--
-- Function bodies are 0162's, patched by anchored replacement — only the
-- emp_late CTE and the delayedEmployerTransfers branch differ. THIS MIGRATION
-- MUST APPLY AFTER 0162.
-- ============================================================================


-- ─────────────────────────────────────────────────────────────────────────────
-- 1) get_admin_attention — count on the same anchor the list uses
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- 0163 — "has never posted a run" is not the same fact as "is overdue", and
  -- treating them as one produced a row the drill-down could say nothing about:
  -- dueBy and daysLate were both derived from lr.last_run, so an employer with
  -- no run was counted here and then listed with three empty columns.
  --
  -- The due date now falls back to the employer's FIRST MEMBER ENROLMENT: the
  -- first payroll was owed one cadence after they had staff to run it for.
  -- An employer with neither a run nor a member has nobody to pay and is no
  -- longer counted at all — previously every such employer was permanently
  -- overdue for a payroll it never owed.
  emp_late AS (
    SELECT count(*) AS n
    ...
    LEFT JOIN LATERAL (
      SELECT min(s.registered_date)::date AS first_member
      FROM public.subscribers s
      WHERE s.employer_id = e.id
    ) fm ON TRUE
    WHERE COALESCE(e.status, 'active') <> 'inactive'
      AND COALESCE(lr.last_run::date, fm.first_member) IS NOT NULL
      AND COALESCE(lr.last_run::date, fm.first_member)
            < v_today - public._employer_grace_days(e.payroll_cadence)
  ),
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 2) get_admin_attention_rows — a real due date for a never-run employer
-- ─────────────────────────────────────────────────────────────────────────────
    ...
    SELECT jsonb_agg(x) INTO v_result FROM (
      SELECT jsonb_build_object(
        ...
        'secondary', COALESCE(initcap(e.payroll_cadence), 'Monthly') || ' payroll · ' ||
                     COALESCE(to_char(lr.last_run, 'DD Mon YYYY'), 'never run'),
        'amount', NULL,
        -- `date` stays NULL for an employer that has never run: there is no
        -- raised date to report, and inventing one would be a lie. The client
        -- renders `neverRun` as "No run" rather than a bare em dash, so the
        -- cell reads as a known state instead of missing data.
        'date', lr.last_run::date,
        'neverRun', (lr.last_run IS NULL),
        -- dueBy/daysLate now come off the same anchor the WHERE uses, so a
        -- never-run employer gets a REAL due date (one cadence after its first
        -- member enrolled) instead of three empty columns.
        'dueBy', (COALESCE(lr.last_run::date, fm.first_member)
                    + public._employer_grace_days(e.payroll_cadence)),
        'daysLate', v_today - (COALESCE(lr.last_run::date, fm.first_member)
                                 + public._employer_grace_days(e.payroll_cadence)),
      ...
      WHERE COALESCE(e.status, 'active') <> 'inactive'
        ...
        AND COALESCE(lr.last_run::date, fm.first_member)
              < v_today - public._employer_grace_days(e.payroll_cadence)
      -- Most overdue first, which is what the tile has always claimed. The old
      -- `lr.last_run NULLS FIRST` put every never-run employer at the top in
      -- arbitrary order and could not rank them against each other at all.
      ORDER BY (COALESCE(lr.last_run::date, fm.first_member)
                  + public._employer_grace_days(e.payroll_cadence))
      LIMIT v_limit
    ) q;
