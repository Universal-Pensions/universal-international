-- 0082_scope_rollup_rpcs.down.sql
-- Restores 0057's get_entity_metrics_rollup verbatim.
-- ⚠️ Reverting re-opens BOTH defects 0082 closed:
--   1. The country level goes back to platform-wide for a distributor, so the
--      Overview KPI will again disagree with the RLS-scoped Subscribers list
--      (0081 stays in force — RLS cannot reach a SECURITY DEFINER function).
--   2. `activeSubscribers` disappears from the payload; the four client call
--      sites fall back to reconstructing a headcount from the ROUNDed percent.
-- Revert 0081 as well if you need the pre-scoping behaviour end to end.
