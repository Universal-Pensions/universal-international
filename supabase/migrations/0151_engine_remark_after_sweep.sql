-- 0151_engine_remark_after_sweep.sql
-- ============================================================================
-- A MONEY BUG IN 0147's PRICING ENGINE, found by auditing my own work.
--
-- THE DEFECT
-- ----------
-- The engine's steps ran in this order inside each loop iteration:
--     (c) allocate or liquidate at the DEALING date's price
--     (d) resync bucket units
--     (e) RE-MARK the member to the BOOK carrying price
--     (f) claim the ledger row
--     (g) run the 0072 save-to-cover sweep, selling units at the DEALING price
--
-- Step (g) moves `units` and `total_balance` AFTER step (e) has already put the
-- member back on the book — and it sells at the dealing price while the member
-- is now carried at the book price. The two are only equal on the normal path.
-- On a BACK-DATED release they differ, and the member is left off-book by
--
--     target x (book_price / dealing_price - 1)
--
-- Step (e) exists precisely to prevent that, and step (g) undid it.
--
-- MEASURED, not reasoned: with the book at 1585.88 and a hole back-filled at
-- 1538.30 (-3%), a 60,000 UGX sweep left member empe-001 off-book by
-- **1,855 UGX** and raised a `nav_mismatch` reconciliation exception. Predicted
-- 1,856; the gap is rounding.
--
-- WHY IT MATTERED MORE THAN THE NUMBER SUGGESTS. The whole point of the
-- re-mark is that `nav_mismatch` asserts `|total_balance - round(units x
-- latest_nav())| <= 1` per member, and `assert_book_revaluable()` refuses to
-- publish when the aggregate implied price drifts >2%. A per-member break is
-- invisible to the aggregate gate (it was measured at 0.0000% drift for a
-- comparable break), so this would have surfaced as a member seeing a wrong
-- balance and a reconciliation row nobody could explain.
--
-- THE FIX
-- -------
-- The re-mark becomes the LAST balance-changing operation in the iteration:
--     (c) allocate/liquidate -> (d) resync -> (f) claim -> (g) sweep -> (h) RE-MARK
-- Nothing may move units or balances after it. The sweep still sells at the
-- dealing price, which is correct and consistent with the contribution that
-- triggered it; the re-mark then restores the on-book relation over whatever
-- the sweep did. `_resync_bucket_units` still precedes the re-mark - the sweep
-- calls it too - so the re-mark never reads a stale unit split.
--
-- ALSO FIXED: `nav_as_of` was stamped with the DEALING date even when the
-- member was then re-marked to the book price, so the column named a valuation
-- day the balance did not reflect. The re-mark now stamps the book's own date.
--
-- ROLLBACK: 0151_engine_remark_after_sweep.down.sql restores 0147's body,
-- reinstating the defect. It says so.
-- ============================================================================
