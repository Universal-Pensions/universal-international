-- 0153_reverse_withdrawal_sign.sql
-- ============================================================================
-- reverse_transaction() HAD NEVER WORKED FOR A WITHDRAWAL. It raised a check
-- constraint violation every time. Found by auditing 0148, not by a report -
-- Phase 7 verified the reversal path only on a CONTRIBUTION, and the withdrawal
-- half was never exercised.
--
-- WHY IT FAILED
-- Withdrawal amounts are stored NEGATIVE (0114), and
-- `transactions_withdrawal_sign_chk` enforces that:
--     CHECK (type <> 'withdrawal' OR (amount <= 0 AND amount > '-Infinity'))
-- The purpose is that SUM(amount) over the ledger is a true net figure.
--
-- reverse_transaction wrote its compensating row with the ORIGINAL type and the
-- amount negated. Reversing a withdrawal is money coming BACK, so that row is
-- type='withdrawal' with a POSITIVE amount — which the constraint refuses.
-- Every attempt to reverse a redemption aborted with:
--     new row for relation "transactions" violates check constraint
--     "transactions_withdrawal_sign_chk"
--
-- THE FIX
-- The compensating row is now type 'reversal'. It matches neither money
-- trigger's WHEN clause, is not in the pricing engine's candidate types, and is
-- subject to no sign constraint, so it can carry either direction honestly.
-- Its splits are signed to agree with its amount. SUM(amount) stays true, and
-- the ledger remains append-only.
--
-- KNOWN CONSEQUENCE, stated rather than hidden: aggregates that filter on
-- `type = 'contribution'` or `type = 'withdrawal'` do not see reversal rows.
-- That is correct — a reversal is not a new contribution — but it means such an
-- aggregate still counts the ORIGINAL row, which is flagged
-- pricing_status = 'reversed'. Any total that must exclude undone money has to
-- filter on pricing_status, not on type. No such aggregate exists today
-- (nothing has ever been reversed), but the next one written must know this.
--
-- ROLLBACK: 0153_reverse_withdrawal_sign.down.sql restores the previous body,
-- which cannot reverse a withdrawal.
-- ============================================================================
