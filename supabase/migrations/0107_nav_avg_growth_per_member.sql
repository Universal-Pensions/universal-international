-- =============================================================================
-- Universal Pensions Uganda — 0107: report AVERAGE growth per member
-- =============================================================================
-- WHY
--   The admin Unit-price page showed a "Growth for members" tile carrying the
--   POOLED figure — total growth divided by total cost basis. That is a
--   money-weighted number: a handful of large, long-tenured balances pull it
--   away from what a typical member actually sees on their own dashboard, so it
--   reads as a placeholder rather than a fact about members.
--
--   This adds `avgGrowthPct` — the mean of each member own growth% — and the
--   supporting `membersWithBasis` count. Measured live 2026-08-08: pooled 9.63%
--   vs average-per-member 9.38%.
--
--   The pooled `growthPct` and `totalGrowth` keys are KEPT. Total growth in
--   shillings is still the honest way to say how much money the fund has made,
--   and removing keys would break any reader mid-flight.
--
-- ⚠️ MEMBERS WITH NO COST BASIS ARE EXCLUDED FROM THE MEAN, not counted as 0%.
--   Counting them would drag the average toward zero without any member ever
--   experiencing that number.
--
-- Generated by transforming the CURRENT LIVE definition with two substitutions,
-- each asserting a unique match, plus guards that the admin gate, SECURITY
-- DEFINER and the pinned search_path all survive. Nothing else moved.
--
