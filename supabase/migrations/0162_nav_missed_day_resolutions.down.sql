-- 0162_nav_missed_day_resolutions.down.sql
-- ============================================================================
-- Reverses 0162.
--
-- Restores both attention functions to their 0116 bodies VERBATIM (the state
-- that was live before 0162), then drops the RPC and the table. Dropping the
-- table first would leave both functions referencing a missing relation, so the
-- order here matters: functions, then function, then table.
--
-- Note this DISCARDS every resolution record. That is the correct behaviour for
-- a down migration — the table is the only place those live — but it means a
-- rollback re-raises every previously-resolved day in the badge.
-- ============================================================================
...
DROP FUNCTION IF EXISTS public.resolve_nav_missed_day(date, text, text);
DROP TABLE    IF EXISTS public.nav_missed_day_resolutions;
