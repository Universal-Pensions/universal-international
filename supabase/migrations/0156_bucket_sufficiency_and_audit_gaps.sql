-- 0156_bucket_sufficiency_and_audit_gaps.sql
-- ============================================================================
-- THREE DEFECTS FOUND BY AUDITING 0147-0155. The first creates phantom money.
--
-- 1. THE LIQUIDATION CLAMPS ABSORBED A PER-BUCKET SHORTFALL IN SILENCE
--
--    The engine's D8 guard checks that the member still holds enough UNITS in
--    total. It never checked that each leg still fits the POT it is drawn from.
--    But the hold is frozen at the amount the member was promised, while their
--    pots are revalued by every publish — so a price FALL between request and
--    dealing date can leave the requested pot worth less than its leg while the
--    member still holds plenty of units overall.
--
--    The total-units check then passes, the leg is applied, and the
--    GREATEST(0, ...) clamps quietly absorb the difference: total_balance falls
--    by only what the pot actually held, while `units` falls by the full
--    amount/price. The member is left carrying value that no units back.
--
--    MEASURED on live, in a rolled-back probe: a 30% price fall against a
--    362,567 UGX hold on a 381,650 UGX savings pot left one member
--    **95,411 UGX above units x price**, raised a nav_mismatch exception, and
--    produced no other signal at all — bucket_sum held, no pot went negative,
--    and the aggregate publish gate was untroubled.
--
--    Fixed by extending the D8 branch to per-bucket sufficiency. Rejecting is
--    the consistent answer and the one D8 already gives for the total: the
--    amount the member confirmed can no longer come from the pot they chose, so
--    tell them and release the hold rather than pay out something else.
--
-- 2. reverse_transaction COULD NEVER REVERSE A SYNCHRONOUS WITHDRAWAL
--
--    0144 deliberately recorded a PRICE but no UNIT MOVEMENT on the synchronous
--    withdrawal path, reasoning that seeded rows never debit units so a delta
--    would be a lie. That reasoning is obsolete: the seed inserts with
--    session_replication_role='replica', so this trigger never sees a seeded row
--    at all, and request_withdrawal is the only runtime writer of a withdrawal
--    (audit F4).
--
--    The cost was that every withdrawal made while the kill switch is off — i.e.
--    every withdrawal on the platform today — is stamped 'priced' with a NULL
--    units_delta, and reverse_transaction refuses exactly those rows with
--    "predates the pricing audit trail". 0153 fixed reversal for withdrawals in
--    principle; this is what makes it reachable in practice.
--
-- 3. nav_missing_days' `status` COLUMN COULD ONLY EVER SAY 'unpriced'
--
--    The LEFT JOIN was restricted to status='published' and the query then kept
--    only the rows where it matched nothing, so the joined row was always NULL
--    and COALESCE always fell through to the literal. Dead weight presented as
--    information — and it hid the distinction an operator needs: a day with a
--    PENDING row is waiting on an admin to confirm a price the fund already
--    sent; a day with no row is waiting on the fund. Four such days exist on
--    live right now and all four reported 'unpriced'.
--
-- ROLLBACK: 0156_bucket_sufficiency_and_audit_gaps.down.sql restores all three
-- prior bodies, reinstating the phantom-balance path.
-- ============================================================================
...
DECLARE
  ...
  v_price       NUMERIC;                     -- 0156
  v_units_before NUMERIC;                    -- 0156
...
BEGIN
  ...
  IF v_async THEN
    ...
    UPDATE public.transactions
     ...
     WHERE id = NEW.id;
  ELSE
    SELECT units INTO v_units_before FROM public.subscriber_balances
     WHERE subscriber_id = NEW.subscriber_id;
    v_price := public.latest_nav();
    ...
    -- 0156: record the UNIT MOVEMENT too, not just the price.
    --
    -- request_withdrawal is about to remove LEAST(amount/price, units) from this
    -- member — this trigger runs before that statement, so `units` read here is
    -- the pre-debit holding and the figure below is exactly what it will take.
    --
    -- Without it every synchronous withdrawal is stamped 'priced' while
    -- recording no unit movement, and reverse_transaction() — which refuses any
    -- row whose units_delta is NULL — could never reverse a single one. That is
    -- every withdrawal made while the kill switch is off, i.e. all of them today.
    --
    -- Safe because request_withdrawal is the ONLY runtime writer of a
    -- type='withdrawal' row (audit F4), and the seed inserts with
    -- session_replication_role='replica', so this trigger never sees a seeded row.
    UPDATE public.transactions
       SET unit_price_applied = v_price,
           units_delta        = CASE WHEN v_price > 0
                                  THEN -LEAST(v_amount / v_price, COALESCE(v_units_before, 0))
                                  ELSE NULL END,
           priced_at          = now()
     WHERE id = NEW.id;
  END IF;
...
AS $function$
  ...
  -- 0156: the join was restricted to status='published' AND then filtered to
  -- rows where it found nothing, so `n` was ALWAYS NULL and this column could
  -- only ever read 'unpriced'. It was dead weight presented as information, and
  -- it hid the distinction that matters operationally: a day with a PENDING row
  -- is waiting on an admin to confirm a price the fund already sent, while a day
  -- with no row at all is waiting on the fund. Join on the date alone, and
  -- report what is actually there.
  SELECT d.nav_date, COALESCE(n.status, 'unpriced')::text
    FROM days d
    LEFT JOIN public.nav_snapshots n
           ON n.fund_code = p_fund AND n.nav_date = d.nav_date
   WHERE n.id IS NULL OR n.status <> 'published'
   ORDER BY d.nav_date;
$function$;
