-- 0086_scope_top_entities_and_lock_anon.down.sql
-- ⚠️ Restores 0077's unscoped get_top_entities (top branches/agents ranked
-- across the whole platform, and a non-deterministic top-N on sort-key ties)
-- and hands anon back EXECUTE on the mutating RPC surface. Every one of those
-- RPCs is still role-gated in-body, so this is a defence-in-depth regression
-- rather than an open door — but there is no reason to run it.
...
GRANT EXECUTE ON FUNCTION public.set_distributor_status(text, text)          TO anon;
GRANT EXECUTE ON FUNCTION public.set_employer_status(text, text)             TO anon;
GRANT EXECUTE ON FUNCTION public.set_commission_rate(numeric)                TO anon;
GRANT EXECUTE ON FUNCTION public.create_distributor(text, text, text, text, text) TO anon;
GRANT EXECUTE ON FUNCTION public.create_employer(text, text, text, text, text, text, text, text, jsonb) TO anon;
GRANT EXECUTE ON FUNCTION public.get_entity_metrics_rollup(text, text[])     TO anon;
GRANT EXECUTE ON FUNCTION public.get_agent_commission_detail(text)           TO anon;
GRANT EXECUTE ON FUNCTION public.search_entities(text)                       TO anon;
GRANT EXECUTE ON FUNCTION public.get_top_branch(text, text)                  TO anon;
