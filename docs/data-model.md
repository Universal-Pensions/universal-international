...
- **Product naming.** The stored ids are `life | health | funeral` and never change — they are the enum in `subscriber_insurance_products.product`, the product check in every RLS policy and RPC, and the value on every existing row. The **display names** are separate: `health` is presented to every role as **"Hospital cash"** (`INSURANCE_PRODUCTS[].label` in `src/constants/savings.js`, `PRODUCT_LABEL`/`PRODUCT_SHORT` in `src/utils/policies.js`). Never map a UI label back to an id by string.
- **Cover amounts are client-chosen from a fixed ladder.** Each product sells four cover levels (`coverTiers(id)`, `src/constants/savings.js`), selectable at self-signup, at agent onboarding, from the subscriber's schedule editor, and on the settings cover page. The DB stores whatever cover/premium the client sends — `_validate_signup_payload` and `fund_insurance_products` validate only `>= 0` and the product enum — so that table is the only pricing authority. Tier 0 of each ladder is the fixed cover the product shipped with before amounts were selectable, which is what keeps legacy payloads byte-identical.
- **Hospital cash pays a daily benefit, not a lump sum.** Its `cover` is the TOTAL across `HOSPITAL_CASH_DAYS` (20) days of hospitalisation, so the figure the member actually receives per night is `cover ÷ 20` (`dailyBenefit(productId, cover)`). Every hospital-cash tier divides evenly: 3M→150,000, 5M→250,000, 8M→400,000, 12M→600,000 per day. Life and funeral pay a single lump sum and have no `benefitDays`, so `dailyBenefit` returns `null` for them.
- **Claims carry a product (`0099`), and WHO may claim depends on it.** `claims.product` ∈ `life | health | funeral` is the discriminator; the legacy `type` column holds a pre-0099 incident category on old rows and mirrors `product` on new ones. **Hospital cash is the only product a living member can claim** — they file it themselves in-app, and `submit_hospital_cash_claim` prices it server-side (`round(cover/20) × payable_nights`, capped at 20 nights per policy year, counting every non-rejected prior claim keyed on admission date). Hospital-cash rows also carry `discharge_date`, `nights`, `provider` and `daily_benefit`; `nights`/`daily_benefit` are stored, not derived, so the amount stays explainable after a later repricing.
- **Death benefits are claimed by a NOMINEE, in a different table.** Life and funeral pay out because the member has died, so the claimant has no account and cannot appear in `claims` (which is keyed on `subscriber_id`). They file at the public `/claim` form, which writes `nominee_claims` (`0100`) via the service role. That table holds the deceased's identifiers (NIN **or** phone — one is required), the claimant's identity and relationship, a human-quotable `reference`, a `pending | in_review | approved | rejected` status, and a nullable `matched_subscriber_id` an admin fills in by hand. Matching is deliberately a human step: an automatic phone/NIN lookup would let anyone probe whether a given person holds a policy.
- **"Amount invested" / investment growth is REAL and stored (since migrations `0103`–`0106`).** The subscriber Home "Amount invested" KPI and the invested-vs-grown story on HomeMobile / HomeDesktop come from `deriveInvestmentGrowth(subscriber)` in `src/utils/finance.js`, which now simply reports `growth = netBalance − invested` from two stored columns. `subscriber_balances.total_balance` is MARKET VALUE (the member's units priced at the fund's current unit price) and `subscriber_balances.invested` is their COST BASIS — what they actually paid in. ⚠️ **This function used to INVENT the number**, discounting the balance backwards over a synthetic tenure (`registeredDate` + a per-`id` jitter, clamped to 3..36 months) at `MONTHLY_RATE`; because the clamp floored at 3 months and 3 months at 10%/yr is exactly +2.5%, a member who signed up and paid once immediately saw "+2.5% growth". It existed because the schema had no cost-basis column and the unit price was frozen at 1,000 UGX, so real growth was structurally always zero. Do not reintroduce a derivation here — `src/test/nav-pricing-contract.test.js` guards the SQL side. **Cost basis falls by the same FRACTION of units redeemed on a withdrawal** (average-cost), so growth% is invariant to withdrawals; reducing it by simple net cash-in instead produces −233% outliers for heavy withdrawers (measured against the live ledger, not hypothesised). **Growth can be negative** — the unit price falls as well as rises.
- **Employer members (two-leg contribution model, `0062` → `0092` → `0093`).** When `employerId` is set, the member's pension is funded by the employer's two-leg contribution run computed from `compensation` — NOT a self-set saving amount (so `contributionSchedule.amount = 0`, `monthlyContribution` is vestigial). The two legs are **independent**: each is a percentage of the member's monthly compensation, and the employer leg is a share of **PAY** — never a share of the employee leg. A run posts ONE `transactions` row **per non-zero leg** — the employee leg as `source='own'`, the employer leg as `source='employer'` — both with `agent_id` NULL (no commission) and `contribution_run_id` set. So a member receives two rows, one row, or (on a 0/0 config) none, in which case they are skipped with `zero_contribution`. ⚠️ Because the employer deducts and remits the employee leg out of payroll, **`source='own'` alone does NOT mean the member paid it themselves** — an `own` row carrying a `contribution_run_id` is a payroll deduction, which is why every member-facing surface splits on that id (`isRunPosted`, `src/utils/periodSettlement.js`) and not on `source`. The employer sets/edits `compensation` via `update_employer_member_compensation` (RPC, `0062`); the member reads their own funding rates via `get_my_employer_funding` (RPC, `0092`) — the only path a subscriber JWT has to the employer's config or name. Shape + math: [Contribution Config shape](#contribution-config-shape).
...
> **PERCENT-ONLY TWO-LEG MODEL (migration `0093`) — the contract of record.** ONE model, **no modes and no per-leg basis**. The employer sets **two independent legs**, each a percentage of the member's monthly `compensation`. The employer leg is a share of **PAY** and is **never** a function of the employee leg. Which sides participate (staff only / staff and company / company only) is **DERIVED** from the two percentages by `contributionParticipants` and is never stored — a stored discriminator is exactly what the deleted `mode` key was. `mode`, `employerMatchPct` and `matchPct` are **DELETED** — both the `0062` `co-contribution`/`employer-only` shapes and the earlier `0038` `matchPct`/`maxContribution` match-of-self-saving shape are retired. The words "co-contribution", "employer-only" and the funding sense of "match" are deliberately absent from the config, the code and every user-facing string.
>
> **Why `0092`:** under `0062` the employer leg was `round(employeeLeg × employerMatchPct/100)`. An employer who said "staff put in 10% and we add 5%" stored `employerMatchPct = 5`, and every run funded `comp × 10% × 5%` = **0.5% of pay — a tenth of what they meant.**
>
> **Why `0093`:** `0092`'s per-leg BASIS (`employeeBasis`/`employerBasis`, each `'percent'|'fixed'`, with `employeeAmount`/`employerAmount` partners) forced the settings screen to ask "flat amount or share of pay?" before it could ask for the figure — four controls and two conditional fields for a choice **no employer had ever used**, on this database or in any seed. `0093` deletes the basis and its flat-amount partners and backfills every row.

The single company-wide `default_contribution_config` JSONB (**there is no per-employee override**) carries exactly two pension keys:
...
- **`0093` BACKFILLED every row**, so no live config carries `mode`, `employerMatchPct`, a basis or a flat amount. A legacy `{ mode:'co-contribution', employeePct:10, employerMatchPct:50 }` still converts money-preservingly at read time (→ `employerPct: 5`; 10% of pay with a 50% match of that leg **is** 5% of pay), but that branch is now restore-from-backup insurance rather than a live path. `0093` **RAISES rather than converting** if it finds a non-zero flat amount — a flat amount cannot be re-expressed as a percentage without each member's pay, so there is no honest automatic conversion.
- **Both write paths validate** (`0093`): `create_employer` AND `update_employer_profile` call the shared `_assert_contribution_config_shape` — the config must be a JSON object and a present percentage must be a JSON **number** in 0-100 (`'{}'` stays legal). Before `0093` only `create_employer` validated, so an employer-role JWT could store `employerPct: 999` and the next run applied it verbatim.

**Two-leg run math** — `submit_employer_contribution_run` (`0093`, superseding the `0062`/`0066`/`0067`/`0092` bodies) re-derives every figure server-side per ACTIVE member from `compensation` (`comp`) + the member's `retirement_pct` (`retPct`, default 80):
