...
|---|---|---|---|---|---|---|---|---|
| `A05-001` | critical | yes | S | **ACTION** | P1 | P1-demo | CLOSED · db7a545, eb616a2 | `05-commission-settlement.md` · apply_settlement has no tenancy check — any distributor can settle another distributor's agents' commissions |
| `A05-002` | critical | yes | S | **ACTION** | P2 | P2-data | CLOSED · owner of A11-001 · 4700154 | `05-commission-settlement.md` · Agent demo persona's Commissions page shows Playwright test residue as real payment history, with two contradictory outstanding-balance figures |
| `A06-001` | critical | yes | M | **ACTION** | P2 | P2-data | CLOSED · owner of A14-002 · 94d75b7 | `06-data-integrity.md` · 61% of the default employer persona's roster balance is uncleaned E2E test money |
...
| `A11-002` | critical | yes | S | **ACTION** | P1 | P1-demo | CLOSED · 493e90c | `11-agent.md` · Agent onboarding wizard cannot complete — final create RPC returns 409 (mock OCR's constant NIN collides with a unique index), trapping the rep on a 'Not saved' screen |
...
| `A22-001` | critical | yes | S | **ACTION** | P1 | P1-demo | CLOSED · bb1d27c | `22-state-errors.md` · Cross-tenant cache bleed: login never clears the React Query cache, so an in-SPA role switch shows the previous role's RLS-scoped money |
| `A24-001` | critical | yes | S | **ACTION** | P1 | P1-demo | CLOSED · 08f854f | `24-frontend-security.md` · Insurance policy certificate can never open — window.open(..., 'noopener') always returns null |
| `A02-001` | high | no | S | **ACTION** | P3 | P3-rls | CLOSED · 18497a0, 799ce0c | `02-rls-matrix.md` · Subscriber JWT can mint arbitrary money by POSTing straight to /rest/v1/transactions |
| `A02-002` | high | no | M | **ACTION** | P3 | P3-rls | CLOSED · 18497a0 | `02-rls-matrix.md` · Subscriber can rewrite their own insurance cover, premium and status to anything (insurance_policies + subscriber_insurance_products) |
| `A03-001` | high | no | S | **ACTION** | P3 | P3-rls | CLOSED · f0959ee | `03-privilege-surface.md` · Anon invite-completion RPC is not bound to the invited phone → cross-tenant subscriber re-tag + compensation overwrite via a shareable token |
| `A04-001` | high | yes | S | **ACTION** | P3 | P3-money | CLOSED · 477fa28, c6fa97d | `04-money-engine.md` · make_contribution accepts NaN / Infinity / unbounded amounts; NaN irrecoverably poisons units and every AUM figure on the platform |
| `A04-002` | high | yes | S | **ACTION** | P3 | P3-money | CLOSED · c6fa97d | `04-money-engine.md` · request_withdrawal validates only that the split legs SUM to the amount, so a negative leg creates money in the retirement bucket |
| `A04-003` | high | yes | S | **ACTION** | P3 | P3-money | CLOSED · 21f8404, 2a2c797 | `04-money-engine.md` · A reseed leaves units at the dead 1,000 UGX price and zeroes bucket units; the next NAV publish inflates AUM 57% and zeroes every member's retirement pot |
| `A05-003` | high | yes | S | **ACTION** | P2 | P2-data | CLOSED · 952c484 | `05-commission-settlement.md` · settlement_batches.paid_amount / line_count do not equal the lines they flipped — 25,000 UGX and 5 lines of settlement are unbacked in live data |
| `A05-004` | high | yes | S | **ACTION** | P3 | P3-money | CLOSED · e07ddd1 | `05-commission-settlement.md` · Re-uploading the same settlement file settles another tranche against the same payment reference — the nonce is minted per file-pick |
| `A05-005` | high | yes | S | **ACTION** | P3 | P3-money | CLOSED · e07ddd1 | `05-commission-settlement.md` · Two rows for the same agent in one settlement upload settle that agent twice in a single call |
| `A06-002` | high | yes | S | **ACTION** | P2 | P2-data | CLOSED · 8c2ddca, c311d21, d978afd | `06-data-integrity.md` · E2E contribution-run cleanup orphans 1,824 transactions on a premise the schema refutes |
| `A06-003` | high | yes | S | **ACTION** | P2 | P2-data | CLOSED · 21f8404, afc889f | `06-data-integrity.md` · Seed's stale MOCK_NOW mirror pushes every contribution schedule 36 days too far out; weekly savers are next due in 8 weeks |
| `A06-004` | high | yes | S | **ACTION** | P3 | P3-money | CLOSED · owner of A11-005 · 88c8110, 8fe2dd7, b31b682 | `06-data-integrity.md` · Agent and subscriber surfaces disagree on 1,284 members' policy status (Active vs Expired) |
| `A06-005` | high | yes | S | **ACTION** | P3 | P3-rls | CLOSED · 4d78a33, 63a41ee | `06-data-integrity.md` · create_employer / create_distributor ignore the identity-write failure, so a new tenant's owner signs in to Nile Breweries |
| `A09-001` | high | yes | S | **ACTION** | P0 | P0-safety | CLOSED · 37c9303 | `09-infra-deploy.md` · Keepalive monitor cannot see or prevent the Supabase auto-pause that takes the demo down |
| `A09-002` | high | no | M | **ACTION** | P7 | P7-tests | CLOSED · 4ccc060 | `09-infra-deploy.md` · Playwright E2E job times out on every push to main, so the §15-M1 db guard has never executed and nothing gates production |
| `A09-003` | high | yes | S | **ACTION** | P0 | P0-safety | CLOSED · 30a22ad | `09-infra-deploy.md` · `npm run seed` TRUNCATEs the live demo database with no confirmation and no backup |
| `A10-001` | high | yes | S | **ACTION** | P4 | P4-dash | CLOSED · 306af4f, 745a934 | `10-subscriber.md` · All Transactions and Annual Tax Statement reports show no data / all zeros in live mode (currentSubscriber never carries transactions) |
...
| `A13-001` | high | yes | S | **ACTION** | P4 | P4-dash | CLOSED · 5130f86, fa0b74b | `13-distributor.md` · Distributor Reports route unreachable on every viewport below 1024px; Menu 'Reports' tile and both routed report screens are dead |
| `A14-001` | high | yes | M | **ACTION** | P4 | P4-dash | CLOSED · 95adbed, 9727064 | `14-employer.md` · "Total contributions" computed from two irreconcilable sources; same screen shows figures differing up to 11.6x, and Runs page states a provably false total |
| `A15-001` | high | yes | M | **ACTION** | P4 | P4-dash | CLOSED · 4f357a4, 6be8299 | `15-admin.md` · Mobile subscriber detail shows every member's Balance / Contributions / Withdrawals as "—" though the member holds real money |
| `A25-004` | high | yes | M | **ACTION** | P7 | P7-tests | CLOSED · 8c2ddca | `25-test-coverage.md` · E2E teardown leaks fixture rows into the LIVE demo DB, incl. 'E2E Branch' rows under d-001 |
| `A26-001` | high | no | S | **ACTION** | P7 | P7-docs | CLOSED · 957c01e | `26-documentation.md` · Four documents assert that RLS blocks direct client writes; it does not, and the shipped frontend writes directly |
| `A26-002` | high | no | S | **ACTION** | P7 | P7-docs | CLOSED · b27dde1 | `26-documentation.md` · api-contracts.md instructs an agent to apply migration 0092 to live; it is already applied |
| `A26-004` | high | no | M | **ACTION** | P7 | P7-docs | CLOSED · eb616a2 | `26-documentation.md` · docs/role-permissions.md disagrees with the measured RLS matrix in seven places, twice contradicting itself |
| `A02-003` | medium | no | S | **ACTION** | P3 | P3-rls | CLOSED · 18497a0 | `02-rls-matrix.md` · contribution_schedules.insurance_funding_mode and the accrual counters are directly writable by the subscriber (spec check 7 violated) |
| `A02-004` | medium | no | S | **ACTION** | P3 | P3-rls | CLOSED · 18497a0 | `02-rls-matrix.md` · Subscriber can create withdrawals and nominees rows directly, bypassing request_withdrawal and its balance/nonce checks |
| `A02-005` | medium | no | M | **ACTION** | P7 | P7-docs | CLOSED · 18497a0 | `02-rls-matrix.md` · agent / branch / distributor can create and edit their own hierarchy rows directly, bypassing the creation RPCs (BACKEND.md says this is impossible) |
| `A04-004` | medium | yes | S | **ACTION** | P3 | P3-money | CLOSED · c6fa97d | `04-money-engine.md` · request_withdrawal with bucket='emergency' for more than the emergency balance clamps the bucket at 0 but debits the full total, so the two buckets stop summing to the total |
| `A04-005` | medium | yes | S | **ACTION** | P3 | P3-money | CLOSED · 2a2c797 | `04-money-engine.md` · publish_nav_snapshot's p_unit_price <= 0 guard AND the unit_price > 0 CHECK constraint both pass NaN/Infinity; with confirmMove the entire book goes NaN |
| `A04-006` | medium | no | M | **ACTION** | P3 | P3-money | CLOSED · ef2c3d2 | `04-money-engine.md` · Four unrelated down-migrations CREATE OR REPLACE the contribution trigger with the hardcoded 1,000 unit price, silently reverting NAV pricing |
| `A04-007` | medium | yes | M | **ACTION** | P3 | P3-money | CLOSED · 2a2c797 | `04-money-engine.md` · NAV is 16 days stale and the 'Delayed NAV updation' counter cannot see it — it counts only pre-seeded pending rows, so 11 unpriced weekdays are invisible |
| `A04-008` | medium | no | S | **ACTION** | P3 | P3-money | CLOSED · 2a2c797 | `04-money-engine.md` · v_reconciliation_exceptions checks the shilling split but not the unit ledger, so a broken units invariant is invisible to the admin |
| `A04-009` | medium | yes | M | **ACTION** | P2 | P2-data | CLOSED · 94d75b7 | `04-money-engine.md` · 33 leftover E2E employer contribution runs (1,881 rows, 145.37M UGX) permanently inflate live AUM, growing ~3.7M every full-suite run |
| `A04-010` | medium | yes | S | **ACTION** | P2 | P2-data | CLOSED · 8c2ddca, 952c484, d978afd | `04-money-engine.md` · Four leftover E2E subscribers named 'TST tree member' / 'TST retag probe' surface on the admin Needs Attention panel during a live demo |
| `A04-011` | medium | no | S | **ACTION** | P3 | P3-money | CLOSED · 493e90c, e07ddd1 | `04-money-engine.md` · The idempotency nonce is claimed AFTER the money write with ON CONFLICT DO NOTHING, so two concurrent same-nonce calls both apply the contribution |
| `A05-006` | medium | no | S | **ACTION** | P3 | P3-money | CLOSED · e07ddd1 | `05-commission-settlement.md` · apply_settlement has no NULL guard on amountPaid — a row with no amount settles the agent's ENTIRE due slice |
| `A05-007` | medium | no | S | **ACTION** | P3 | P3-money | CLOSED · e07ddd1 | `05-commission-settlement.md` · Over-payment is silently swallowed — the entered amount above the due total is recorded nowhere and produces no skip or warning |
| `A05-008` | medium | yes | S | **ACTION** | P2 | P2-data | CLOSED · 952c484 | `05-commission-settlement.md` · Seeded settlement batches are stamped with the wrong branch, so the b-kam-015 demo branch persona is told UGX 45,000 was paid while its own Commissions page shows 0 paid and a 0% settlement rate |
| `A05-009` | medium | no | M | **ACTION** | P3 | P3-money | CLOSED · ef2c3d2 | `05-commission-settlement.md` · 0089's down-migration would silently revert NAV pricing — it re-emits trg_transactions_contribution with the hardcoded 1,000 UGX unit price |
| `A06-006` | medium | yes | S | **ACTION** | P2 | P2-data | CLOSED · owner of A15-003 · 94d75b7 | `06-data-integrity.md` · Four abandoned E2E fixtures are 4 of the 7 rows on the Admin Reconciliation screen |
| `A06-008` | medium | no | S | **ACTION** | P7 | P7-tests | CLOSED · afc889f | `06-data-integrity.md` · DB invariant #5 is vacuous by 41 days and blind to 21 NULL rows |
| `A06-009` | medium | yes | M | **ACTION** | P2 | P2-data | CLOSED · afc889f | `06-data-integrity.md` · A fifth clock, public._demo_now() = 2026-05-18, is live in SQL and 44 days behind the JS anchor |
| `A06-010` | medium | no | S | **ACTION** | P7 | P7-tests | CLOSED · 8c2ddca, 952c484, d978afd | `06-data-integrity.md` · SUBSCRIBER_CHILD_TABLES misses three subscriber-FK tables; two of them have no FK at all |
| `A06-011` | medium | yes | S | **ACTION** | P2 | P2-data | CLOSED · 4d78a33 | `06-data-integrity.md` · The only employer created through the live approval path has an empty config, NULL cadence and a district ID in the district-name field |
| `A07-001` | medium | n/r | n/r | **ACTION** | P6 | P6-sec | CLOSED · d42e542 | `07-api-auth.md` · Sentry scrubber has no NIN redaction pattern |
| `A09-004` | medium | no | M | **ACTION** | P6 | P6-sec | CLOSED · 214aa00 | `09-infra-deploy.md` · Enforcing the report-only CSP would break the app's typography three ways and the KYC ID previews; today the policy has no report sink so it collects nothing |
| `A09-005` | medium | no | S | **ACTION** | P6 | P6-infra | CLOSED · d42e542 | `09-infra-deploy.md` · Frontend Sentry is not configured in production — @sentry/react is tree-shaken out of the shipped bundle entirely |
| `A09-006` | medium | no | S | **ACTION** | P6 | P6-infra | CLOSED · 922ca90, ff5d8e3 | `09-infra-deploy.md` · render.yaml is not the applied configuration — the live build command has drifted from the blueprint |
| `A09-007` | medium | no | S | **ACTION** | P0 | P0-safety | CLOSED · 37c9303 | `09-infra-deploy.md` · Keepalive fires roughly a third as often as its own stated design rationale requires |
| `A09-008` | medium | no | M | **ACTION** | P6 | P6-sec | CLOSED · 9b9bcb9 | `09-infra-deploy.md` · Dependabot security alerts are disabled, and 12 version PRs are wedged behind a red lint gate |
| `A09-009` | medium | no | M | **ACTION** | P0 | P0-safety | CLOSED · 957c01e, ce01384 | `09-infra-deploy.md` · No documented rollback for the frontend or the API, and 22 migrations cannot be reversed at all |
| `A10-002` | medium | yes | S | **ACTION** | P4 | P4-dash | CLOSED · 745a934 | `10-subscriber.md` · Insurance settings shows '0 beneficiaries on file' when a beneficiary exists (currentSubscriber never carries nominees) |
| `A11-003` | medium | yes | S | **ACTION** | P4 | P4-dash | CLOSED · 60c3364, b528a15 | `11-agent.md` · Desktop agent home shows scheduled monthly-equivalent (UGX 331K) but labels it 'What members saved this month'; mobile shows actual collected (UGX 291K) |
| `A11-004` | medium | yes | S | **ACTION** | P4 | P4-dash | CLOSED · 60c3364, b31b682, b528a15 | `11-agent.md` · Agent Settings renders a malformed phone with a double country code (+256 256 711 443975) |
| `A11-006` | medium | yes | S | **ACTION** | P4 | P4-dash | CLOSED · b31b682 | `11-agent.md` · 'Yet to contribute' flashes the entire roster (11) before the contributions query resolves, then settles to the correct 1 — no loading gate on the dependency |
| `A12-001` | medium | yes | S | **ACTION** | P4 | P4-dash | CLOSED · 21f8404, 60c3364, afc889f, b528a15 | `12-branch.md` · Branch collections charts drift against the demo clock — wall-clock month labels over MOCK_NOW-anchored data |
| `A12-002` | medium | yes | S | **ACTION** | P4 | P4-dash | CLOSED · b528a15 | `12-branch.md` · Agent-detail gender donut prints percentages as a subscriber count ('100 subscribers') |
| `A12-003` | medium | yes | M | **ACTION** | P4 | P4-dash | CLOSED · 60c3364, b528a15 | `12-branch.md` · District rank computed from a stale stored score, not the 'recomputed daily' gauge (84 live vs 77 stored) |
| `A12-005` | medium | yes | M | **ACTION** | P4 | P4-dash | CLOSED · 5130f86 | `12-branch.md` · Per-agent subscriber list is desktop-only — unreachable on mobile |
| `A14-003` | medium | yes | S | **ACTION** | P4 | P4-dash | CLOSED · 60c3364, 95adbed, b528a15 | `14-employer.md` · Expired invites labeled "awaiting sign-up" on Overview & roster, contradicting Pending KYC page ("0 awaiting · 4 lapsed") |
| `A15-002` | medium | yes | S | **ACTION** | P4 | P4-dash | CLOSED · ac57832, c8d8195 | `15-admin.md` · Admin platform hero has no error/retry state; a failed money read renders "—" / 0 subscribers / "Needs work" silently |
...
| `A16-001` | medium | yes | S | **ACTION** | P5 | P5-ux | CLOSED · owner of A18-004 · dc1dbac | `16-public-onboarding.md` · Mobile public pages (FAQ/Contact/About/Request-access) render with no <h1>; About starts at <h3> |
| `A17-001` | medium | yes | M | **ACTION** | P5 | P5-ux | CLOSED · 5a3f5d6 | `17-design-system.md` · Circular avatars violate the standing "no circular avatars" house rule and are internally inconsistent |
...
| `A17-003` | medium | no | M | **ACTION** | P5 | P5-ux | CLOSED · 66f1f0b | `17-design-system.md` · Four dashboard BottomSheets declare aria-modal but implement no focus trap — behavioural divergence from the hardened landing copy |
| `A18-001` | medium | yes | S | **ACTION** | P5 | P5-ux | CLOSED · 5a3f5d6 | `18-mobile-pwa.md` · iOS Safari zoom-on-focus: dashboard/search/payment inputs render below 16px |
...
| `A18-003` | medium | yes | S | **ACTION** | P5 | P5-ux | CLOSED · 66f1f0b | `18-mobile-pwa.md` · Bottom sheets and PaySheet do not lock body scroll; page scrolls behind an open sheet on mobile |
...
| `A19-004` | medium | yes | M | **ACTION** | P6 | P6-infra | CLOSED · 306af4f | `19-desktop-shells.md` · Distributor + admin Subscribers (~4,602) / Agents (~2,046) lists defeat their virtualizer in dash mode |
...
| `A20-001` | medium | yes | S | **ACTION** | P5 | P5-ux | CLOSED · 5a3f5d6 | `20-accessibility.md` · Public site footer nav links render invisible (1.35:1) on desktop landing pages |
| `A20-002` | medium | yes | M | **ACTION** | P5 | P5-ux | CLOSED · 5a3f5d6 | `20-accessibility.md` · Widespread AA contrast failures incl. status pills and the green money value on Save/Withdraw |
| `A20-003` | medium | no | M | **ACTION** | P5 | P5-ux | CLOSED · 66f1f0b | `20-accessibility.md` · aria-modal dialogs without focus trap/restore (incl. PaySheet payment surface) |
| `A20-004` | medium | no | S | **ACTION** | P5 | P5-ux | CLOSED · 66f1f0b | `20-accessibility.md` · Closed landing nav drawer keeps focusable children tabbable while aria-hidden |
| `A20-005` | medium | no | S | **ACTION** | P5 | P5-ux | CLOSED · cdc66b7 | `20-accessibility.md` · Horizontally/vertically scrollable data tables are not keyboard-accessible |
| `A21-001` | medium | yes | M | **ACTION** | P6 | P6-infra | CLOSED · 42af888, 4f357a4 | `21-performance.md` · Distributor/admin subscriber list downloads the entire collection client-side (~3.4MB raw / ~6,765 rows into memory) |
| `A22-002` | medium | yes | M | **ACTION** | P4 | P4-dash | CLOSED · ac57832, c8d8195 | `22-state-errors.md` · Primary dashboard hero money reads have no error/retry state — a read failure renders 'FUNDS UNDER MANAGEMENT —' / 0 subscribers / 'Health Score 0 Needs work' silently |
...
| `A22-004` | medium | yes | M | **ACTION** | P4 | P4-dash | CLOSED · ac57832 | `22-state-errors.md` · Raw technical error strings ('TypeError: Failed to fetch', raw Postgres exception text) leak into user toasts on writes; the friendly fallback never wins |
| `A22-005` | medium | yes | S | **ACTION** | P4 | P4-dash | CLOSED · 0650205 | `22-state-errors.md` · Access-request approve/reject does not invalidate adminAttention, so the 'Pending access requests' count stays stale up to 5 minutes |
| `A24-002` | medium | no | S | **ACTION** | P6 | P6-sec | CLOSED · 214aa00 | `24-frontend-security.md` · CSP is report-only, reports nowhere, and cannot be enforced as written — 58 violations if flipped on |
| `A25-001` | medium | yes | L | **ACTION** | P7 | P7-tests | CLOSED · 5124e7e | `25-test-coverage.md` · Baseline Playwright suite ships RED with 30 deterministic failures (28 reproduced), not flake |
...
| `A25-006` | medium | no | S | **ACTION** | P7 | P7-tests | CLOSED · 1f8985b | `25-test-coverage.md` · api/, server/, e2e/ TypeScript (100 files) is not linted at all |
| `A25-007` | medium | no | S | **ACTION** | P7 | P7-tests | CLOSED · 1f8985b | `25-test-coverage.md` · No typecheck script; tsc checks 32 of 100 .ts files and skips all tests + all e2e |
| `A25-009` | medium | no | S | **ACTION** | P7 | P7-tests | CLOSED · 1f8985b | `25-test-coverage.md` · All 34 jsx-a11y rules forced to 'warn' and lint has no --max-warnings; 311 a11y warnings hide in a green build |
| `A25-011` | medium | no | S | **ACTION** | P7 | P7-tests | CLOSED · 4ccc060 | `25-test-coverage.md` · CI section 15-M1 'executed-not-skipped' guard runs only on push-to-main, not on PRs — the surface that gates merges |
...
| `A26-003` | medium | no | S | **ACTION** | P4+P7 | P4-dash + P7-docs | CLOSED · 957c01e, afc889f | `26-documentation.md` · MOCK_NOW documented as 2026-05-26 in four docs; the real value is 2026-07-01, and two code copies drifted 36 days from it |
| `A26-005` | medium | no | M | **ACTION** | P7 | P7-docs | CLOSED · 1f8985b | `26-documentation.md` · Twelve of CLAUDE.md's thirteen 'binding' rules have no mechanical enforcement; one is already violated |
...
| `A02-006` | low | no | S | **ACTION** | P3 | P3-rls | CLOSED · 18497a0 | `02-rls-matrix.md` · subscriber_insurance_products has no SELECT policy for branch, distributor, employer or admin -- 1,473 live rows invisible to four roles |
| `A02-007` | low | no | S | **ACTION** | P7 | P7-docs | CLOSED · eb616a2 | `02-rls-matrix.md` · distributors is unreadable by subscriber / agent / branch / employer, contradicting docs/role-permissions.md |
| `A02-008` | low | no | S | **ACTION** | P3 | P3-rls | CLOSED · owner of A24-003 · 18497a0 | `02-rls-matrix.md` · anon reads of 15 tables return HTTP 401 naming an internal helper function instead of an empty result set |
| `A03-002` | low | no | S | **ACTION** | P3 | P3-rls | CLOSED · f0959ee | `03-privilege-surface.md` · Anon signup RPC trusts the client for phone canonicalization; a non-canonical phone makes the created account unreachable at login (falls to s-0001 default) |
| `A03-003` | low | no | S | **ACTION** | P3 | P3-rls | CLOSED · f0959ee | `03-privilege-surface.md` · No length cap on subscriber text fields; anon signup RPC persists unbounded field values |
| `A04-012` | low | no | S | **ACTION** | P3 | P3-money | CLOSED · c6fa97d | `04-money-engine.md` · MIN_CONTRIBUTION / MIN_WITHDRAW of 5,000 and the zero-decimal UGX rule are client-only; the RPCs accept 1 UGX and 0.004 UGX |
| `A04-013` | low | no | S | **ACTION** | P3 | P3-money | CLOSED · owner of A06-014 · c6fa97d | `04-money-engine.md` · request_withdrawal writes a POSITIVE transactions.amount while all 5,402 historical withdrawal rows are negative |
| `A04-014` | low | yes | S | **ACTION** | P4 | P4-dash | CLOSED · 2a2c797 | `04-money-engine.md` · The admin publish form computes the price move against currentNav while the RPC computes it against the price preceding p_nav_date; for a back-dated publish the two disagree |
| `A04-015` | low | no | S | **ACTION** | P4 | P4-dash | CLOSED · 2a2c797 | `04-money-engine.md` · The NAV publish form's default date uses UTC, so between 00:00 and 03:00 East Africa Time it defaults to yesterday |
| `A05-010` | low | yes | S | **ACTION** | P4 | P4-dash | CLOSED · 4700154 | `05-commission-settlement.md` · The agent's partial-settlement banner shows an internal 34-character batch UUID instead of the human payment reference |
| `A05-011` | low | no | S | **ACTION** | P1 | P1-demo | CLOSED · db7a545 | `05-commission-settlement.md` · parseAmount misparses formula and scientific-notation money cells into plausible-looking amounts, and returns 0 in violation of its own contract |
| `A05-012` | low | no | S | **ACTION** | P3 | P3-money | CLOSED · e07ddd1 | `05-commission-settlement.md` · A commission rate of 0 is accepted and generates UGX 0 commission rows instead of no commission |
| `A05-013` | low | no | M | **ACTION** | P3 | P3-money | CLOSED · e07ddd1 | `05-commission-settlement.md` · Re-assigning a subscriber to a different agent generates a second commission for the same subscriber |
| `A06-012` | low | yes | S | **ACTION** | P2 | P2-data | CLOSED · 4d78a33 | `06-data-integrity.md` · Six of eight employers and distributor d-003 have no sign-in path |
| `A06-013` | low | no | S | **ACTION** | P2 | P2-data | CLOSED · 4d78a33, 5038abe | `06-data-integrity.md` · 39 of 54 users rows carry no entity_id |
...
| `A06-015` | low | yes | S | **ACTION** | P4 | P4-dash | CLOSED · bcd6776 | `06-data-integrity.md` · 21 employer-member contribution schedules have amount = 0 and a NULL next_due_date |
| `A06-016` | low | no | M | **DEFER** | — | — | DEFERRED | `06-data-integrity.md` · Mass-detach guard passes 50-row batches unjournalled |
| `A06-017` | low | yes | S | **ACTION** | P2 | P2-data | CLOSED · 952c484 | `06-data-integrity.md` · E2E entity residue across six tables, including a branch invisible to every distributor |
| `A07-002` | low | n/r | n/r | **ACTION** | P6 | P6-sec | CLOSED · 4f11d4b | `07-api-auth.md` · agent-referral unauthenticated service-role INSERT (input-capped) |
| `A07-004` | low | n/r | n/r | **ACTION** | P6 | P6-sec | CLOSED · 4f11d4b | `07-api-auth.md` · Rate limiter IP-spoofable via X-Forwarded-For in no-proxy deployment (local only; prod trust-proxy:1 mitigates) |
| `A09-010` | low | no | S | **ACTION** | P6 | P6-infra | CLOSED · 4f11d4b | `09-infra-deploy.md` · /healthz and /readyz are registered before morgan, so all health and readiness traffic is invisible in the logs |
| `A09-011` | low | no | S | **ACTION** | P6 | P6-infra | CLOSED · 1f8985b | `09-infra-deploy.md` · CI installs with --legacy-peer-deps while Render installs with plain npm ci; the flag is unnecessary and masks future peer breaks |
| `A09-012` | low | no | S | **ACTION** | P6 | P6-infra | CLOSED · owner of A24-005 · 5124e7e, d42e542 | `09-infra-deploy.md` · @sentry/react is a devDependency but is imported by code that ships to the browser |
| `A09-013` | low | no | S | **ACTION** | P7 | P7-tests | CLOSED · 1f8985b | `09-infra-deploy.md` · The repo's only typecheck skips every API test file; there is no root tsconfig and no pre-commit hook |
| `A09-014` | low | no | S | **ACTION** | P6 | P6-infra | CLOSED · 223d592 | `09-infra-deploy.md` · SUPABASE_URL is absent from .env.local and local dev survives only on a fallback the code says is marked for removal |
| `A10-004` | low | no | S | **ACTION** | P5 | P5-ux | CLOSED · 306af4f, 745a934 | `10-subscriber.md` · On mobile, all 5 report sub-views share the same <h1> ('Analytics') |
| `A11-007` | low | yes | S | **ACTION** | P4 | P4-dash | CLOSED · 21f8404, 60c3364, afc889f, b528a15 | `11-agent.md` · 'This month' resolves to two different calendar months on the same agent dashboard (Contributions=June 2026, Onboarded=August 2026) |
| `A12-004` | low | no | M | **ACTION** | P4 | P4-dash | CLOSED · 5130f86 | `12-branch.md` · Mobile 'reports' redirect lands on the overview instead of analytics (desktop is correct) |
...
| `A12-007` | low | yes | S | **ACTION** | P4 | P4-dash | CLOSED · b528a15 | `12-branch.md` · Branch Settings 'Save changes' and mobile 'Update password' fabricate success without persisting |
| `A12-008` | low | yes | S | **ACTION** | P4 | P4-dash | CLOSED · 60c3364, b528a15 | `12-branch.md` · Absurd '▲ 14903% over the year' on the branch overview & analytics |
| `A13-002` | low | yes | S | **ACTION** | P4 | P4-dash | CLOSED · owner of A15-004 · b31b682 | `13-distributor.md` · Distributor mobile Branches/Agents lists show all-zero metrics (0 subs / 0 agents / 0 funds) for ~2-3s until the separate metrics query resolves |
...
| `A14-004` | low | yes | S | **ACTION** | P4 | P4-dash | CLOSED · 60c3364, b528a15 | `14-employer.md` · Overview "Needs attention" mislabels combined group cover as "Group life cover" |
...
| `A16-002` | low | yes | S | **ACTION** | P5 | P5-ux | CLOSED · 306af4f | `16-public-onboarding.md` · FAQ/Contact/About have no signup CTA in the 769-920px band; Navbar hamburger drawer is dead code |
...
| `A17-005` | low | yes | S | **ACTION** | P5 | P5-ux | CLOSED · 83886df | `17-design-system.md` · Cross-role inconsistency: the "Ask AI" affordance is styled two different ways |
| `A17-006` | low | yes | M | **ACTION** | P5 | P5-ux | CLOSED · 83886df | `17-design-system.md` · Cross-role inconsistency: KPI tiles use two different patterns (left accent border vs flat) |
...
| `A17-008` | low | no | S | **ACTION** | P5 | P5-ux | CLOSED · 5a3f5d6 | `17-design-system.md` · Decorative CSS keyframe animations ignore prefers-reduced-motion |
...
| `A18-005` | low | no | S | **ACTION** | P5 | P5-ux | CLOSED · 5a3f5d6 | `18-mobile-pwa.md` · Touch targets below 44x44: app-bar back/icon buttons (40px), copilot close (32px) |
| `A18-006` | low | yes | S | **ACTION** | P5 | P5-ux | CLOSED · 306af4f | `18-mobile-pwa.md` · Two distinct subscriber report routes share the generic app-bar title 'Analytics' on mobile |
...
| `A20-006` | low | no | S | **ACTION** | P5 | P5-ux | CLOSED · cdc66b7 | `20-accessibility.md` · Skip-link target <main id="main"> is not programmatically focusable |
| `A20-007` | low | no | M | **ACTION** | P5 | P5-ux | CLOSED · 1f8985b | `20-accessibility.md` · 310 untracked jsx-a11y warnings dominated by two over-strict rules; real defects ~16 |
...
| `A20-009` | low | no | S | **ACTION** | P5 | P5-ux | CLOSED · 6478435 | `20-accessibility.md` · 6 autoFocus usages can disorient keyboard/screen-reader users |
| `A20-011` | low | no | S | **ACTION** | P5 | P5-ux | CLOSED · 306af4f, 6478435 | `20-accessibility.md` · Subscriber balance change is not announced to screen readers |
...
| `A21-003` | low | no | S | **ACTION** | P6 | P6-infra | CLOSED · ff5d8e3 | `21-performance.md` · Employer demo seed data (22KB gzip) ships to the browser on the live-backend path |
| `A21-004` | low | no | S | **DEFER** | — | — | DEFERRED | `21-performance.md` · Redundant/duplicate indexes and minor DB advisor lints |
| `A22-006` | low | yes | S | **ACTION** | P4 | P4-dash | CLOSED · 0650205, 9dfd4d7 | `22-state-errors.md` · Support ticket confirms 'sent to your agent' then silently vanishes on refresh (Open 3→2, no error copy) |
...
| `A24-004` | low | no | S | **ACTION** | P6 | P6-sec | CLOSED · 25767fa, 9b9bcb9 | `24-frontend-security.md` · react-router 7.17.0 carries 5 advisories incl. a high; the fix (7.18.2) is inside the declared semver range |
...
| `A24-011` | low | no | S | **ACTION** | P6 | P6-sec | CLOSED · 223d592 | `24-frontend-security.md` · xlsx resolves from cdn.sheetjs.com, not the npm registry — every build depends on that CDN being reachable |
| `A25-008` | low | no | M | **DEFER** | — | — | DEFERRED | `25-test-coverage.md` · No stylelint, no import-boundary rule, no pre-commit hooks |
| `A25-010` | low | no | S | **ACTION** | P7 | P7-tests | CLOSED · 1f8985b | `25-test-coverage.md` · ESLint lints 66 untracked/.gitignored files; no-unused-vars is 'error', so a stray scratch file can fail the gate |
...
| `A26-012` | low | no | S | **ACTION** | P7 | P7-docs | CLOSED · 957c01e | `26-documentation.md` · README.md still lists 'hardcoded unit price' as intentional demo scope; migrations 0103-0106 retired it |
| `A26-013` | low | no | S | **ACTION** | P7 | P7-docs | CLOSED · 37c9303 | `26-documentation.md` · render-operational.md states the keepalive cadence wrongly in three places, carries two dead external references, and leaves a completed cutover instruction standing |
| `A26-014` | low | no | S | **ACTION** | P7 | P7-docs | CLOSED · 957c01e | `26-documentation.md` · Seed-entity counts drifted across CLAUDE.md, SPEC.md, data-model.md and role-permissions.md; the third distributor is documented nowhere |
| `A02-009` | info | no | S | **KEEP** | — | — | NO-ACTION | `02-rls-matrix.md` · report says: "suggested_fix: No code change." |
| `A02-010` | info | no | S | **ACTION** | P2 | P2-data | CLOSED · 952c484 | `02-rls-matrix.md` · One branches row has distributor_id IS NULL and is invisible to every distributor |
...
| `A03-005` | info | no | S | **ACTION** | P7 | P7-docs | CLOSED · 957c01e | `03-privilege-surface.md` · Dead commission-run migration 0021_commission_rpcs_app_role.sql retained in repo with no down-migration |
...
| `A04-016` | info | no | S | **ACTION** | P2 | P2-data | CLOSED · 952c484, a737299, ced8dce | `04-money-engine.md` · The single live unit-bucket invariant break on s-0005 is AUDIT-CAUSED, not a product defect — it belongs in the write ledger |
...
| `A05-014` | info | no | S | **ACTION** | P2 | P2-data | CLOSED · 8c2ddca, 952c484 | `05-commission-settlement.md` · settlement_uploads is an unbounded, never-reconciled idempotency ledger claiming 1,530,000 UGX of settlement against 50,000 actually paid |
| `A05-015` | info | no | S | **ACTION** | P3 | P3-money | CLOSED · 18497a0 | `05-commission-settlement.md` · Migration 0087 documents an ownership guard on get_agent_commission_detail that its body never emits |
...
| `A06-020` | info | yes | S | **ACTION** | P2 | P2-data | CLOSED · 2a2c797, 952c484 | `06-data-integrity.md` · Four stale pending NAV snapshots sit behind a later published price |
| `A07-003` | info | n/r | n/r | **KEEP** | — | — | NO-ACTION | `07-api-auth.md` · report says: "impact: none for this deployment" |
| `A09-015` | info | no | S | **ACTION** | P6 | P6-infra | CLOSED · c117cae | `09-infra-deploy.md` · The retired Tokyo Supabase project is still ACTIVE in the same free organisation as the live one |
| `A09-016` | info | no | S | **KEEP** | — | — | NO-ACTION | `09-infra-deploy.md` · report says: "suggested_fix: No action needed for the demo." |
| `A09-017` | info | no | S | **ACTION** | P6 | P6-sec | CLOSED · 4f11d4b, ff5d8e3 | `09-infra-deploy.md` · Two header-level inconsistencies: content-hashed assets are not served immutable, and the API sets CORP same-origin on a deliberately cross-origin service |
| `A09-018` | info | no | S | **ACTION** | P7 | P7-docs | CLOSED · 4f11d4b | `09-infra-deploy.md` · Environment-documentation gaps and one stale note in docs/BACKEND.md §2 |
| `A10-003` | info | no | S | **ACTION** | P7 | P7-tests | CLOSED · 5124e7e, f36c7fd | `10-subscriber.md` · The 6 baseline mobile subscriber-dashboard Playwright failures are title-divergence (test brittleness), not product defects |
| `A11-008` | info | yes | S | **ACTION** | P4 | P4-dash | CLOSED · 4f11d4b, ff5d8e3 | `11-agent.md` · Agent commissions error card surfaces a raw 'TypeError: Failed to fetch' string |
| `A12-I01` | info | no | S | **ACTION** | P2 | P2-data | CLOSED · 94d75b7 | `12-branch.md` · Two E2E-leftover branches pollute the Kampala district in live data |
| `A12-I02` | info | yes | S | **ACTION** | P4 | P4-dash | CLOSED · 60c3364, b528a15 | `12-branch.md` · Manager-name vs persona-name inconsistency across branch surfaces |
...
| `A16-003` | info | yes | S | **ACTION** | P2 | P2-data | CLOSED · 952c484 | `16-public-onboarding.md` · All seeded employer invites are expired; /invite/:token entry flow is not demoable from seed data |
| `A17-009` | info | yes | S | **ACTION** | P6 | P6-infra | CLOSED · 214aa00 | `17-design-system.md` · Web fonts load async (FOUT/CLS on cold load) |
...
| `A20-010` | info | no | S | **ACTION** | P6 | P6-infra | CLOSED · cdc66b7 | `20-accessibility.md` · <html lang="en"> while every formatter uses en-UG |
...
| `A21-006` | info | yes | M | **ACTION** | P0 | P0-safety | CLOSED · 37c9303 | `21-performance.md` · Free-tier cold start compounds first-paint of the first data screen |
...
| `A22-007` | info | no | S | **ACTION** | P4 | P4-dash | CLOSED · ac57832 | `22-state-errors.md` · No global QueryCache.onError — every read failure is silent unless the consuming component individually guards isError |
| `A24-007` | info | no | S | **ACTION** | P6 | P6-sec | CLOSED · 08f854f | `24-frontend-security.md` · The single raw-HTML sink is correctly escaped today — and it writes into a popup that can read the session token |
| `A24-008` | info | no | M | **KEEP** | — | — | NO-ACTION | `24-frontend-security.md` · report says: "suggested_fix: No action required for security." |
| `A24-009` | info | no | S | **ACTION** | P6 | P6-sec | CLOSED · d42e542 | `24-frontend-security.md` · Sentry is wired end to end but completely inert in production — the SDK is not even in the bundle |
| `A24-010` | info | no | S | **REFUTED** | — | — | NO-ACTION | `24-frontend-security.md` · report says: "suggested_fix: No action." |
| `A25-013` | info | no | S | **ACTION** | P7 | P7-tests | CLOSED · 5124e7e, 5da657e, f36c7fd | `25-test-coverage.md` · Exactly one genuinely flaky spec in the whole suite; the predicted 'prime flake candidates' are real defects |
| `A26-015` | info | no | S | **ACTION** | P7 | P7-docs | CLOSED · 957c01e | `26-documentation.md` · No live doc carries a 'verified against live on <date>' line; docs/role-permissions.md has no temporal marker anywhere in 362 lines |
| `A26-016` | info | no | S | **ACTION** | P7 | P7-docs | CLOSED · 957c01e | `26-documentation.md` · The anon-EXECUTE surface is documented as 3; it is 13, and the sentence contradicts itself |
