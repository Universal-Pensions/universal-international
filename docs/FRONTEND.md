...
- **Pension config tab = two independent legs, no mode radios.** `EmployerSettings` → `PensionContributionTab` renders **two always-visible, structurally identical fieldsets** — "What staff contribute" / "What you contribute" — each a basis radio pair ("A flat amount each month" / "A share of their pay") plus the matching numeric input. Distinct radio groups (`emp-employee-basis` / `emp-employer-basis`) and unique labelled ids (`emp-employee-pct` / `emp-employee-amount` / `emp-employer-pct` / `emp-employer-amount`) — the old `emp-default-mode` / `emp-default-basis` / `emp-default-match` selectors are gone. The draft seed is ONE `normalizeContributionConfig(employer?.defaultContributionConfig)` call (lifted into the shared `SettingsBody` draft so Pension + Insurance share one config), so a `'{}'` employer opens at **0/0 with percent bases** — the form no longer invents a 10% / 50%-match / UGX 50,000 default. Save validates each leg **independently** (percent → finite 0-100; fixed → finite ≥ 0; blank → 0; no cap, no minimum) and always writes all six keys plus the three unchanged insurance keys in ONE `update_employer_profile` mutate (`0056`). **0/0 saves successfully with a non-blocking warning toast** (plus a standing note in the live preview) — it is never hard-blocked and there is no confirm dialog. The preview calls `deriveContributionLegs`, so what the employer sees is what the run posts.
...
- `ContributionSettings.jsx` (1480 lines) — the "Plan & pay" wizard. **Two pages behind a tablist**, not one long form:
  ...
  - **Page 2 "Protect your family"** — multi-select cover switches over `INSURANCE_PRODUCTS` (life/health/funeral, priced **annually** via `annualPremium`), a "cost for one year" total, then a **Route A "Pay now" vs Route B "Save up for it"** radiogroup. Route B adds a savings-split slider (what share of the liquid slice builds cover) plus the `SaveUpTin` coin-fill pace gauge (`tinFillState` / `TIN_LINE_PCT`).
...
| `contributionModel.js` (new) | `CONTRIBUTION_BASES` (`['percent','fixed']`), `normalizeContributionConfig(config)`, **`deriveContributionLegs(config, compensation)`**, `isLegZero(basis, pct, amount)`, `formatLegRate` / `formatLegRateForMember`, `contributionFundingLabel(config)`, `memberFundingSummary(config, employerName)`. **THE single source of truth for the employer's two-leg contribution math and its wording** (unified model, migration `0092`) — see §5 "Contribution-run write path". Two INDEPENDENT legs, each `percent` of the member's monthly compensation or a flat UGX amount; the employer leg is never a function of the employee leg; either leg may be 0 and 0/0 is legal. `normalizeContributionConfig` absorbs the legacy `mode` shapes money-preservingly (`employeePct 10` + `employerMatchPct 50` → `employerPct 5`) and `{}` → all zeros, so **no caller ever reads a raw config key**. ⚠️ **SQL parity obligation:** migration `0092` mirrors `normalizeContributionConfig` (`public._normalize_contribution_config`) and `deriveContributionLegs` (inside `submit_employer_contribution_run`) in PL/pgSQL — the three must change in ONE commit. Pure, no mockData, only imports `formatUGX`. |
