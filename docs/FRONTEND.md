...
**Resolve (0162) — opt-in, NAV only.** Every signal clears by doing the real work (settle the withdrawal, run the payroll, publish the price), and that stays the default. **Delayed NAV updation** is the one exception: a valuation day that will never be priced would otherwise nag forever, and the only alternative was inventing a unit price. A signal opts in by setting `resolvable` in `attentionMeta` **and** joining the `RESOLVERS` registry in `hooks/useAdminAttention.js` — both, or it cannot be resolved. Resolving is **final** (no reopen), so `ResolveComposer` is a confirm dialog rather than an inline button, and its copy must keep saying that it does not set a price. ⚠️ Resolved rows deliberately STAY in the list as the record of which day was missed, so `useAttentionDrill`'s `total` filters them out — `get_admin_attention` already skips them server-side, and counting them on the tile would reproduce the badge-vs-list drift of A04-007.
...
| `attention/` | Needs-attention drill-downs. `attentionMeta.js` (per-signal title/lead/columns/notify draft), `useAttentionDrill.js` (shared data + notify flow + resolve flow), `AdminAttentionDesktop.jsx` + `AdminAttentionMobile.jsx` (+ CSS), `NotifyComposer.jsx` and `ResolveComposer.jsx` (both wrapped in `<Modal>` on desktop and `<BottomSheet>` on mobile; ResolveComposer reuses NotifyComposer's stylesheet rather than duplicating it) |
...
| `adminAttention.js` | Admin "Needs attention" signal feed (`0097` RPCs) + the NAV resolve write (`0162`) | `getAdminAttention()`, `getAdminAttentionRows(type, limit?)`, `resolveNavMissedDay({navDate, note?, fundCode?})`, `EMPTY_ATTENTION` | `useAdminAttention` hooks → admin dashboard attention widgets |
...
| `useAdminAttention.js` | `useAdminAttention()`, `useAdminAttentionRows(type, limit?)`, `useResolveAttentionRow(type)` | `services/adminAttention.js` |
