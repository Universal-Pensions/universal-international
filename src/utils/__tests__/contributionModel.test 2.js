...
describe('rounding', () => {
  ...
  it('rounds a half-shilling up, matching SQL round() for non-negative money', () => {
    // 33,333 × 1.5% = 499.995 → 500 ; 33,300 × 1.5% = 499.5 → 500.
...
describe('formatLegRate / formatLegRateForMember', () => {
  ...
  it('is compensation-free, so a rate can be shown without disclosing anyone pay', () => {
    // Neither function takes a compensation argument at all — the property that
    // lets the employer roster show a rate beside a member it may not price.
    expect(formatLegRate.length).toBe(3);
    expect(formatLegRateForMember.length).toBe(3);
  });
});
