...
- **`smoke/`** (9 tracked files as of 2026-08-25: the original 8 — every dashboard route × **all 6 roles** [subscriber, agent, branch, distributor, **employer**, **admin**] + landing + a `_health` check — plus `landing-nav-band.spec.ts`. Mobile-specific smoke specs are being added as part of ongoing mobile work; `ls e2e/specs/smoke/` may already show more than 9 by the time this is read — this directory changes fastest of the four). Each route loads, renders its identity element, and shows its primary CTA without crashing.
...
- **`db/`** (5 files as of 2026-08-25, up from 4) — service-role DB-contract specs with no browser: `invariants.spec.ts` (schema/business invariants), `money-idempotency.spec.ts` (no double-credit on retried writes), `rls-isolation.spec.ts` (cross-tenant RLS isolation), `deactivate-entities.spec.ts` (entity deactivate/reactivate journalling, `0060`/`0080`), and `function-deployment-contract.spec.ts` (new this pass — not yet described here; read the spec file for its coverage before relying on this line).
...
7. **`/dashboard/reports/contributions`** (subscriber dashboard) — `REPORT_VIEWS` map keys the route segment as `contributions-summary` not `contributions`. The legacy URL `/dashboard/reports/contributions` would 404; the spec hits the corrected slug.
